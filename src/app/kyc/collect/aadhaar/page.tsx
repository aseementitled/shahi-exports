...
export default function AadhaarCollectionPage() {
  ...
  // Check if Aadhaar is already uploaded and redirect if so
  React.useEffect(() => {
    const kycData = localStorage.getItem('kycData');
    if (kycData) {
      const parsed = JSON.parse(kycData);
      if (parsed.aadhaarDocument && parsed.aadhaarUploaded) {
        // Aadhaar already uploaded, redirect to next missing document
  ...
  const handleSkip = () => {
    ...
    console.log('Skipping Aadhaar, saving data:', updatedKycData); // Debug log
    localStorage.setItem('kycData', JSON.stringify(updatedKycData));
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
            ...
            {!capturedImage ? (
              <div className="space-y-4">
                <div className="text-center">
                  {!isCapturing && (
                    <h3 className="text-lg font-semibold text-black mb-4">
                      {t('uploadFile', 'kyc')} {t('aadhaarCard', 'kyc')}
                    </h3>
                  )}
              ...
              <div className="space-y-4">
                <div className="text-center">
                  ...
                  <p className="text-green-600 font-medium mb-4">
                    {t('fileSelected', 'kyc')} {t('aadhaarCard', 'kyc')}
                  </p>
        ...
        <div className="mt-8 text-center space-y-2">
          <button
            onClick={() => router.push('/kyc/collect/pan')}
